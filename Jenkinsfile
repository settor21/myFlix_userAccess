    ...
    stages {
        ...
        stage('Dockerize Application') {
         ...
         stage('Redeploy Container to Web') {
            steps {
                script {
                    sh "ssh -o StrictHostKeyChecking=no ${PROD_USERNAME}@${PROD_SERVER} 'cd myflix/user-access && docker stop ${DOCKER_CONTAINER_NAME} || echo \"Failed to stop container\"'"
                    sh "ssh -o StrictHostKeyChecking=no ${PROD_USERNAME}@${PROD_SERVER} 'cd myflix/user-access && docker rm ${DOCKER_CONTAINER_NAME} || echo \"Failed to remove container\"'"
